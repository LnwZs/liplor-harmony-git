    ...
    if ret:
        ...
        if len(faces) > 0:
            for face in faces:
                ...
                # Apply lip color to the lips region only
                lips_mask = np.zeros_like(imgGray)  # Create a mask for lips region
                cv2.fillPoly(lips_mask, [myPoints[48:61]], 255)  # Fill lips region with white

                # Apply lip color to the lips region only
                imgColorLips = np.zeros_like(frame_rgb)
                imgColorLips[:, :, 0] = red_var.get()  # Blue channel
                imgColorLips[:, :, 1] = green_var.get()  # Green channel
                imgColorLips[:, :, 2] = blue_var.get()  # Red channel
                imgColorLips = cv2.bitwise_and(imgColorLips, imgColorLips, mask=lips_mask)  # Apply mask

                # Blend lip color into the lips region of frame_rgb
                frame_rgb = cv2.addWeighted(frame_rgb, 1.0, imgColorLips, 0.6, 0)
