...
def update_webcam():
    ...
    if len(faces) > 0:
             for face in faces:
                 ...
                 black_lips = np.zeros_like(frame_rgb)

                 # Create a mask for the lip region
                 mask = np.zeros(frame_rgb.shape[:2], dtype=np.uint8)
                 cv2.fillPoly(mask, [myPoints], (255, 255, 255, 255))  # Fill the lip region with white color

                 # Set the lip region in the black image to (0, 0, 0)
                 black_lips[mask == 255] = (0, 0, 0, 255)

                 # Invert the mask
                 inverted_mask = cv2.bitwise_not(mask)

                 # Initialize a new image with the same size and type as 'frame_rgb'
                 result_image = np.zeros_like(frame_rgb)

                 # Where the mask is True (255), copy pixels from 'other_image' to 'result_image'
                 result_image[mask == 255] = imgColorLips[mask == 255]

                 # Where the mask is False, copy pixels from 'frame_rgb' to 'result_image'
                 result_image[inverted_mask == 255] = frame_rgb[inverted_mask == 255]
                 ...
                 #imgColorLips = cv2.add(black_lips,imgColorLips)
