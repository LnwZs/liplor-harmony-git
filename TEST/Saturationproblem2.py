...
def update_webcam():
    ...
    if len(faces) > 0:
             for face in faces:
                 ...
                 lipcolora = np.zeros_like(imgLips)
                 ...
                 #lipColor = cv2.bitwise_and(imgLips, adjustment)
                 #imgColorLips = cv2.bitwise_and(lipColor, imgColorLips)
                 
                 lipcolora = cv2.bitwise_and(imgLips, lipcolora)
                 imgColorLips = cv2.bitwise_and(imgLips, imgColorLips)
                 #lipcolora = cv2.bitwise_and(lipcolora, imgColorLips)

                 lipcolora[:, :, 0] = 50   # Blue channel
                 lipcolora[:, :, 1] = 0   # Green channel
                 lipcolora[:, :, 2] = 30   # Red channel

                 imgColorLips = cv2.GaussianBlur(imgColorLips, (7, 7), 9)
                 lipcolora = cv2.GaussianBlur(lipcolora, (7, 7), 9)

                 #imgColorLips = cv2.addWeighted(lipcolora, 1, imgColorLips, 1, 0)
                 imgColorLips = cv2.addWeighted(frame_rgb, 1, imgColorLips, 0.3, 0)
    else:
