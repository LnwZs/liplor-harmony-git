...
def update_webcam():
    ...
    if len(faces) > 0:
        for face in faces:
            ...
            imgColorLips_resized = cv2.resize(imgColorLips, (frame_rgb.shape[1], frame_rgb.shape[0]))

            # Convert to PIL Image
            pil_img = Image.fromarray(imgColorLips_resized)

            # Adjust saturation using ImageEnhance
            saturation_factor = 100  # Example saturation factor, adjust as needed
            enhancer = ImageEnhance.Color(pil_img)
            pil_img = enhancer.enhance(saturation_factor)

            # Convert back to OpenCV format
            imgColorLips_resized = np.array(pil_img)

            # Combine imgColorLips_resized with frame_rgb using blending
            alpha = 1
            beta = 0.3
            blended_img = cv2.addWeighted(frame_rgb, alpha, imgColorLips_resized, beta, 0)

            frame_rgb = blended_img
    else:
