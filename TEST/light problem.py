...
def update_webcam():
    ...
    if len(faces) > 0:
        for face in faces:
            ...
            # Overlay imgColorLips onto frame_rgb
            overlay = imgColorLips[:, :, :3]  # Extract only the RGB channels (excluding the alpha channel)
            mask = imgColorLips[:, :, 3:]  # Extract the alpha channel

            # Resize overlay and mask to match the dimensions of frame_rgb
            overlay_resized = cv2.resize(overlay, (frame_rgb.shape[1], frame_rgb.shape[0]))
            mask_resized = cv2.resize(mask, (frame_rgb.shape[1], frame_rgb.shape[0]))

            # Convert overlay_resized to RGBA by adding an alpha channel
            overlay_resized_rgba = cv2.cvtColor(overlay_resized, cv2.COLOR_RGB2RGBA)

            # Invert the alpha channel
            mask_inv = cv2.bitwise_not(mask_resized)

            # Black-out the area of overlay in ROI
            bg_masked = cv2.bitwise_and(frame_rgb, frame_rgb, mask=mask_inv)

            # Combine the overlay and the background
            try:
                dst = cv2.add(bg_masked, overlay_resized_rgba)
                frame_rgb = dst
            except cv2.error as e:
                print("Error:", e)
                print("Shape of bg_masked:", bg_masked.shape)
                print("Shape of overlay_resized:", overlay_resized_rgba.shape)
...
# Function to display the original webcam feed
def old_pic():
    # Capture the latest frame from the webcam
    ret, frame = cap.read()
    if ret:
